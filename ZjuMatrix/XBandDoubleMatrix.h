...
namespace ZjuMatrix {
    ...
    /// Define a class for symmetric band matrix of double.
	...
	protected:
		...
		/// Description: the position at 1D array for the diagonal element
	...
	public:
		...
		void random();
		XIntVector viSkyline();                             // 返回带状矩阵每行的skyline (return the matrix's skyline)
		XIntVector viDiagonalPos();                         // 返回带状矩阵每行对角线元素在一维数组中的位置 (return the position at 1D array for the diagnal element of each row)
