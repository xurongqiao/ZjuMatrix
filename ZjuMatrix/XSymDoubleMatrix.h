...
namespace ZjuMatrix
{
	/// Define a class for symmetric matrix of double.
	...
	public:
		virtual void ldl();                                   // 对称矩阵的LDL分解，结果覆盖原矩阵(LDL decomposition and this symmetric matrix is overwritten)
	...
	public:
		...
		// -------------------------------------------------------------------------
		friend void operator *= (XSymDoubleMatrix& m, double c);             //矩阵m=矩阵m*标量
	};
}